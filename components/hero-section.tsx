...
export function HeroSection() {
  ...
  return (
    ...
    >
      ...
      {/* Left image panel */}
      <div className="absolute inset-y-0 left-0 w-full lg:w-1/2 pointer-events-none overflow-hidden">
        <div
          className="absolute inset-0 bg-cover bg-center bg-no-repeat"
          style={{ backgroundImage: "url('/images/hero-bg.jpg')" }}
        />
        <div className="absolute inset-0 bg-slate-950/70 lg:bg-gradient-to-r lg:from-slate-950/60 lg:to-transparent" />

        {/* Animated lines SVG */}
        <svg
          className="absolute inset-0 opacity-20 pointer-events-none hidden lg:block z-20"
          viewBox="0 0 600 800"
          fill="none"
        ...
        >
          {Array.from({ length: 12 }).map((_, i) => (
            <path
              key={i}
              d={`M${i * 40} 0 Q${i * 30 + 200} 400 ${i * 35 + 100} 800`}
          ...
          <div className="w-full max-w-xl lg:max-w-lg">
            <div className="max-w-3xl">
              ...
              <ScrollReveal delay={2}>
                <p className="text-lg md:text-2xl text-muted-foreground leading-relaxed mb-12 text-pretty border-l-2 border-primary/20 pl-6">
                  {content.subtitle}
                </p>
              </ScrollReveal>
